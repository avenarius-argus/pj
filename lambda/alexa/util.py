...
class podcast:
    def __init__(self,rss,name):
        self.rf = rss
        self.hand = feedparser.parse(self.rf)
        self.title = name
        self.image=self.hand.feed.image.href
    
    def getEpisodes(self):
        self.episodes = []
        for i in range(len(self.hand.entries)):
            self.episodes.append(self.hand.entries[i].title)
        return self.episodes
    
    def getEpisodeDates(self):
        self.episodeDates = []
        for i in range(len(self.hand.entries)):
            self.episodeDates.append(self.hand.entries[i].published)
        return self.episodeDates
    
    def getaudio(self):
        self.audio = []
        for i in range(len(self.hand.entries)):
            self.audio.append(self.hand.entries[i].enclosures[0].href)
        return self.audio

    def getcertainone(self,i):
        ceraudio =self.audio[i]
        cerepisode = self.episodes[i]
        cerdate = self.episodeDates[i]
        return ceraudio,cerepisode,cerdate
    
    def getrandom(self):
        rand = random.randint(0,len(self.hand.entries))
        return self.getcertainone(rand)
    
    def getlatest(self):
        return self.getcertainone(0)
...
class PlayHandler(AbstractRequestHandler):
    def can_handle(self, handler_input):
        # check if the request type is PlaybackController.PlayCommandIssued
        # or PlaybackController.PauseCommandIssued
        return (is_request_type("PlaybackController.PlayCommandIssued")(handler_input) or
                is_request_type("PlaybackController.PauseCommandIssued")(handler_input))

    def handle(self, handler_input):
        # get the audio item from the request
        audio_item = handler_input.request_envelope.request.audio_item

        # get the stream URL from the audio item
        stream_url = audio_item.stream.url

        # get the play behavior from the audio item
        play_behavior = audio_item.stream.play_behavior

        if play_behavior == PlayBehavior.REPLACE_ALL:
            # if the play behavior is REPLACE_ALL, play the audio from the
